...
const Tooltip = ({ children, ..._props }: any) => React.createElement(React.Fragment, null, children)
...
const TooltipContent = React.forwardRef<any, any>(({ className, children, ..._props }, _ref) =>
  React.createElement(
    'div',
    { className: cn('z-50 rounded-md bg-gray-900 px-3 py-1.5 text-xs text-white shadow-md', className), ..._props },
    children
  )
)
