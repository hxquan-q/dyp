...
function SidebarMenuItem({ children, className, ref }: { children: React.ReactNode; className?: string; ref?: React.Ref<HTMLLIElement> }) {
  return (
...
function UserPanel() {
  ...
  React.useEffect(() => {
    ...
    return open ? (document.addEventListener('mousedown', handler), () => document.removeEventListener('mousedown', handler)) : undefined
  }, [open])
        ...
        <SidebarMenuItem ref={menuRef}>
          <SidebarMenuButton onClick={() => setOpen(!open)} className="gap-3">
            ...
            <div className="flex flex-1 flex-col leading-tight min-w-0">
              <span className="truncate text-sm font-medium text-gray-900">{user.name || '未登录'}</span>
          ...
          {open && (
            <div className="absolute bottom-full left-0 right-0 z-50 mb-1 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
              <div className="flex flex-col p-1 text-sm">
                <button type="button" className="flex items-center gap-2 rounded-md px-3 py-2 text-gray-700 hover:bg-gray-100" onClick={handleUploadLogs}>
...
function TopBar() {
  ...
  return (
    <div className="flex h-14 items-center justify-between border-b border-border bg-background px-4">
      <button type="button" aria-label="Toggle Sidebar" className="rounded-md p-2 text-gray-500 hover:bg-gray-100">
      ...
      <Sidebar>
          ...
          <div className="p-2">
            <Link href="/dashboard" className="flex items-center gap-3 rounded-md p-3">
              ...
              <div className="flex flex-col justify-center">
                <span className="text-base font-bold text-gray-900 tracking-tight ml-0.5">扣单宝</span>
              </div>
            </Link>
          </div>
                        ...
                        <SidebarMenuItem key={item.url}>
                          <Link
                            ...
                            className={cn(
                              ...
                              isActive ? 'bg-gray-200 text-gray-900 hover:bg-gray-300' : 'text-gray-700 hover:bg-gray-100'
                            )}
                          >
        ...
        <div className="mt-auto">
          <SidebarMenu className="space-y-1 px-2 pb-2">
            <SidebarMenuItem>
              <a href="/kefu.png" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-md px-3 py-2.5 text-gray-700 hover:bg-gray-100">
                <MessagesSquare className="w-4 h-4 shrink-0" />
                <span className="text-base font-medium">联系客服</span>
              </a>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <a href="/help" target="_blank" rel="noopener noreferrer" className="flex items-center justify-between rounded-md px-3 py-2.5 text-gray-700 hover:bg-gray-100">
