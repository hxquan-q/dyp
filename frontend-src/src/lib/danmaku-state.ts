...
function normalizeDanmakuItem(raw: any): DanmakuItem {
  const commentId = typeof (raw?.commentId ?? raw?.comment_id) === 'string'
  ...
  return {
    ...
    luckyBagPosition: Number.isFinite(Number(raw?.luckyBagPosition ?? raw?.lucky_bag_position))
      ...
      : null,
    batchNo: typeof (raw?.batchNo ?? raw?.batch_no) === 'string' ? (raw.batchNo ?? raw.batch_no) : null,
    luckyBagBatchNo: typeof (raw?.luckyBagBatchNo ?? raw?.lucky_bag_batch_no) === 'string'
...
export function mergeResolvedItems(state: DanmakuState, resolved: DanmakuItem[]): DanmakuState {
  ...
  if (!activeBatchNo) {
    activeBatchNo = list.find((it) => typeof it.batchNo === 'string' && it.batchNo && !ignored[it.batchNo!])?.batchNo ?? null
  }
  let activeLucky = state.activeLuckyBagBatchNo
  if (!activeLucky) {
    activeLucky = list.find((it) => it.luckyBagBatchNo && !ignoredLucky[it.luckyBagBatchNo!])?.luckyBagBatchNo ?? null
  }
