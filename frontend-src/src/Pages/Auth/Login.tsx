...
// 模块: eH -> 组件函数: JB
function JB() {
  const { csrfToken: e, flash: t } = Xr().props,
  ...
  const J = async (te) => {
      (te.preventDefault(),
        A((Z) => ({
          ...Z,
          _token:
            e || document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
        })));
      const Q = {};
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <span className="inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-2 text-xs font-medium text-orange-700">
              <span className="size-2 rounded-full bg-green-500 animate-pulse" />
              智能直播场控系统
            </span>
            <h1 className="text-4xl lg:text-5xl font-bold leading-tight text-gray-900">欢迎回来</h1>
            <p className="text-lg text-gray-600 max-w-md">
              登录后即可继续管理您的直播店铺、打印配置和订单数据
            </p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100">
              ...
              <p className="font-semibold text-gray-900 text-sm">数据安全隔离</p>
              <p className="text-gray-500 text-xs mt-1">每个租户独立存储，保障数据安全</p>
            </div>
            <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100">
              <div className="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center mb-3">
                <span className="text-lg">🖨️</span>
              </div>
              <p className="font-semibold text-gray-900 text-sm">云端打印同步</p>
              <p className="text-gray-500 text-xs mt-1">支持菜鸟云打印，多端实时同步</p>
            </div>
          </div>
        </div>
        <Ab className="border border-gray-200 bg-white shadow-xl rounded-3xl">
          <Sb className="pb-4">
            <Ob className="text-2xl text-gray-900">{a ? '忘记密码' : '账号登录'}</Ob>
            <Ib className="text-gray-500">
              {a ? '通过手机号验证码重置密码' : '使用您的手机号和密码登录'}
            </Ib>
          </Sb>
          <Lb>
            {(w.phone || w.password || I.phone || I.code || I.password || p || t?.error) && (
              <div className="mb-4 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-600">
                <AlertCircle className="mt-0.5 h-4 w-4" />
                <div>
                  {w.phone && <p>{w.phone}</p>}
                  {w.password && <p>{w.password}</p>}
                  {I.phone && <p>{I.phone}</p>}
                  {I.code && <p>{I.code}</p>}
                  {I.password && <p>{I.password}</p>}
                  {p && <p>{p}</p>}
                  {t?.error && <p>{t.error}</p>}
                </div>
              </div>
            ...
            {(x || t?.success) && (
              <div className="mb-4 flex items-start gap-2 rounded-xl border border-green-200 bg-green-50 p-3 text-sm text-green-600">
                ...
                <div>
                  {x && <p>{x}</p>}
                  {t?.success && <p>{t.success}</p>}
                </div>
              </div>
            )}
            {a ? (
              <form className="space-y-4" onSubmit={de}>
                <div className="space-y-2">
                  <Label htmlFor="reset_phone" className="text-gray-700">
                    手机号
                  </Label>
                  <div className="flex items-center gap-2 rounded-xl bg-gray-50 px-4 border border-gray-200 focus-within:ring-2 focus-within:ring-orange-400 focus-within:border-orange-400 transition-all">
                    ...
                    <Input
                      ...
                      value={R.phone}
                      onChange={(te) => D('phone', te.target.value)}
                      placeholder="请输入手机号"
                      required={true}
                      autoComplete="tel"
                    />
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex flex-1 items-center gap-2 rounded-xl bg-gray-50 px-4 border border-gray-200 focus-within:ring-2 focus-within:ring-orange-400 focus-within:border-orange-400 transition-all">
                      ...
                      <Input
                        ...
                        className="border-0 bg-transparent text-gray-900 placeholder:text-gray-400 focus-visible:ring-0"
                        value={R.code}
                        onChange={(te) => D('code', te.target.value)}
                        placeholder="请输入验证码"
                        required={true}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 rounded-xl bg-gray-50 px-4 border border-gray-200 focus-within:ring-2 focus-within:ring-orange-400 focus-within:border-orange-400 transition-all">
                    ...
                    <Input
                      ...
                      className="border-0 bg-transparent text-gray-900 placeholder:text-gray-400 focus-visible:ring-0"
                      value={R.password}
                      onChange={(te) => D('password', te.target.value)}
                      placeholder="请输入新密码"
                      required={true}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 rounded-xl bg-gray-50 px-4 border border-gray-200 focus-within:ring-2 focus-within:ring-orange-400 focus-within:border-orange-400 transition-all">
                    ...
                    <Input
                      ...
                      onChange={(te) => D('password_confirmation', te.target.value)}
                ...
                <Button
                  type="submit"
                  className="w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-semibold rounded-xl py-6 shadow-lg shadow-orange-500/25"
                  loading={U}
                  loadingText="正在重置..."
                >
                  重置密码
              ...
              <form className="space-y-5" onSubmit={J}>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 rounded-xl bg-gray-50 px-4 border border-gray-200 focus-within:ring-2 focus-within:ring-orange-400 focus-within:border-orange-400 transition-all">
                    ...
                    <Input
                      ...
                      className="border-0 bg-transparent text-gray-900 placeholder:text-gray-400 focus-visible:ring-0"
                      value={_.phone}
                      onChange={(te) => S('phone', te.target.value)}
                      placeholder="请输入手机号"
                      required={true}
                      autoComplete="tel"
                    />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 rounded-xl bg-gray-50 px-4 border border-gray-200 focus-within:ring-2 focus-within:ring-orange-400 focus-within:border-orange-400 transition-all">
                    ...
                    <Input
                      ...
                      className="border-0 bg-transparent text-gray-900 placeholder:text-gray-400 focus-visible:ring-0"
                      value={_.password}
                      onChange={(te) => S('password', te.target.value)}
                      placeholder="请输入密码"
                      required={true}
                ...
                <div className="flex items-center justify-between text-sm">
                  <label className="inline-flex items-center gap-2 text-gray-600">
                    <Checkbox
                      id="remember"
                      checked={_.remember}
                      onCheckedChange={(te) => S('remember', !!te)}
