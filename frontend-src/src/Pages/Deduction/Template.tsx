...
function OM() {
  ...
  const { templates: e = [], flash: t = {} } = Xr().props,
    { showError: a, ErrorToastRenderer: o } = gl(),
    [l, u] = m.useState([]),
  ...
  function ie(Q) {
    ...
    return (
      Q.horizontal &&
        ((Z += Q.horizontal > 0 ? '向右' : '向左'), (Z += Math.abs(Q.horizontal) + 'mm ')),
      Q.vertical && ((Z += Q.vertical > 0 ? '向下' : '向上'), (Z += Math.abs(Q.vertical) + 'mm')),
      Z
    );
  }
  const de = () => {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4 border w-full rounded-lg p-4 bg-white">
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          <div className="2xl:block h-5 w-px bg-gray-300" />
          <Button
            variant="outline"
            onClick={D}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>偏移(横/竖)</TableHead>
              <TableHead>默认打印机</TableHead>
          ...
          <TableBody>
            ...
            {E.map((Q) => (
              <TableRow>
                ...
                <TableCell>{ie(Q)}</TableCell>
                <TableCell>
                  <PrinterSelect
                    value={Q.default_printer}
                    onValueChange={(Z) => B(Q, Z)}
                    printerList={l}
                    width="200px"
                    className="h-8 text-xs"
                    showClearOption={true}
                    refreshing={d}
                    onRefresh={() => R(true, true)}
                  />
                </TableCell>
                <TableCell>
                  <Switch checked={Q.is_default} onCheckedChange={(Z) => J(Q, Z)} />
                </TableCell>
                <TableCell className="text-right space-x-2">
                  <Button variant="link" className="text-blue-600 h-auto p-0" onClick={() => U(Q)}>
                    编辑
                  </Button>
                  <Button
                    variant="link"
      ...
      <SizeDialog open={x} onOpenChange={v} onConfirm={V} onInvalid={a} />
    </div>
  );
}
