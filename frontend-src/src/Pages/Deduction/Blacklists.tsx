...
function BT() {
  ...
  const { items: e = [], tenantId: t = 0, flash: a = {} } = Xr().props,
  ...
  const S = {
    ...
    { data: j, setData: N, reset: w, errors: E } = Co(S),
    [A, R] = m.useState(false),
    { showError: D, ErrorToastRenderer: V } = gl(),
    U = () => {
      (l(false),
        d(null),
        w(),
    ...
    J = (Q) => {
      confirm('确定要删除该黑名单记录吗？') &&
        Et.delete(`/blacklists/${Q}`, {
          preserveScroll: true,
          onSuccess: U,
        });
    },
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4 border w-full rounded-lg p-4 bg-white">
        <div className="flex items-center gap-2">
          ...
          <Select value={v} onValueChange={(Q) => _(Q)}>
            ...
            <SelectContent>
              ...
              {mv.map((Q) => (
                <SelectItem value={Q.value}>{Q.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          <Button className="bg-blue-500 hover:bg-blue-600 w-[90px]" onClick={de}>
            查询
          </Button>
          <Button variant="outline" className="w-[90px]" onClick={te}>
            重置
          </Button>
      ...
      <div className="bg-white rounded-lg border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[220px]">昵称</TableHead>
          ...
          <TableBody>
            ...
            {e.map((Q) => (
              <TableRow>
                <TableCell>{Q.nickname}</TableCell>
                <TableCell>{mv.find((Z) => Z.value === Q.platform_type)?.label}</TableCell>
                <TableCell className="text-right space-x-2">
                  <Button variant="link" className="text-blue-600 h-auto p-0" onClick={() => B(Q)}>
                    编辑
                  </Button>
                  <Button
                    variant="link"
      ...
      <Dialog open={o} onOpenChange={l}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-xl">{u ? '编辑黑名单' : '新增黑名单'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                value={j.nickname}
                maxLength={64}
                onChange={(Q) => N('nickname', Q.target.value)}
                className="h-11"
              />
              {E.nickname && <p className="text-sm text-red-500">{E.nickname}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="platform_type" className="text-base">
                <span className="text-red-500">* </span>平台
              </Label>
              <Select value={j.platform_type} onValueChange={(Q) => N('platform_type', Q)}>
                <SelectTrigger id="platform_type" className="h-11 w-[100%]">
                  <SelectValue placeholder="请选择平台" />
                </SelectTrigger>
                <SelectContent>
                  {mv.map((Q) => (
              ...
              {E.platform_type && <p className="text-sm text-red-500">{E.platform_type}</p>}
            </div>
          </div>
          <DialogActions
            onCancel={U}
